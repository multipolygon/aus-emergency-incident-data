...
const geoJsonSchema = JSON.parse(
  fs.readFileSync(path.join(".", "schema.geo.json"))
);
...
const sources = {
  ...
  sa_cfs: {
    type: "kml",
    url:
      "http://data.eso.sa.gov.au/prod/cfs/criimson/cfs_incident_placemark.xml",
  },
  sa_mfs: {
    type: "kml",
    url:
      "http://data.eso.sa.gov.au/prod/mfs/criimson/mfs_incident_placemark.xml",
  },
  tas: {
    type: "kml",
...
const mapProperties = {
  vic: ({
    ...
    created: moment
      .tz(created, moment.ISO_8601, "Australia/Melbourne")
      .format(),
    updated: moment
      .tz(updated, moment.ISO_8601, "Australia/Melbourne")
      .format(),
    id,
    sourceTitle,
    feedType,
  ...
  nsw: ({ description, guid, title, pubDate }) => {
    const { updated, fire, type, status, location, size } = parseHtmlData(
      description
    );
    return {
      id: guid,
      sourceTitle: title,
      created: moment
        .tz(pubDate, "D/MM/YYYY h:mm:ss A", "Australia/Sydney")
        .format(),
      updated: moment
        .tz(updated, "D MMM YYYY HH:mm", "Australia/Sydney")
        .format(),
      feedType: "incident",
  ...
  wa: ({
    ...
    id: [incidentEventsId, DFESUniqueNumber, messageId]
      .filter(Boolean)
      .join("-"),
    sourceTitle: locationSuburb,
    created: moment
      .tz(startTime, "YYYY-MM-DD HH:mm:ss", "Australia/Perth")
      .format(),
    updated: moment
      .tz(lastUpdatedTime, "YYYY-MM-DD HH:mm:ss", "Australia/Perth")
      .format(),
    feedType: "incident",
    category: type,
    subcategory: type,
  ...
  wa_warn: ({
    ...
    id: [incidentEventsId, DFESUniqueNumber, messageId, fcadId]
      .filter(Boolean)
      .join("-"),
    sourceTitle: locationSuburb,
    created: startTime,
    updated: moment
      .tz(lastUpdatedTime, "DD-MM-YY hh:mm:ss a", "Australia/Perth")
      .format(),
    feedType: "warning",
    category: type,
    subcategory: type,
  ...
  sa_cfs: ({ name, styleUrl, description }, id) => {
    const { "first reported": firstReported, status } = parseHtmlData(
      description,
      "<br>"
    );
  ...
  tas: ({ name, description }, id) => {
    const {
      ...
      status,
      size,
    } = parseTasDescription(description);
    const created = moment
      .tz(lastUpdate, "DD-MMM-YYYY hh:mm a", "Australia/Hobart")
      .format();
    return {
      id: id + name,
      sourceTitle: name,
    ...
    const output = {
      ...
      features: obj.features.map((feature) => ({
        ...
        properties: cleanProperties(
          mapProperties[name](feature.properties, feature.id)
        ),
      })),
    };
...
const convertKmz = (name, src) => {
  ...
  exec(
    `cd ${dirPath} && unzip -o ${name}.kmz doc.kml`,
    (error, stdout, stderr) => {
      if (error || stderr) {
        console.log(error);
        console.log(stderr);
